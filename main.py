    ...
    try:
        ...
        if "organizations" in insp.get_table_names():
            ...
            org_extra = {
                ...
                "billing_waarschuwing_op":          "TIMESTAMP",
            }
        ...
        if "ai_analyses" in insp.get_table_names():
            ...
            gebruiker_cols = [c["name"] for c in insp.get_columns("users")]
            if "schouw_camera" not in gebruiker_cols:
                with engine.begin() as conn:
                    conn.execute(text("ALTER TABLE users ADD COLUMN schouw_camera TEXT"))
                print("[migration] users.schouw_camera toegevoegd.")
            ...
            if "schouw_camera_standaard" not in org_cols:
                with engine.begin() as conn:
                    conn.execute(text(
                        "ALTER TABLE organizations ADD COLUMN schouw_camera_standaard TEXT"))
                print("[migration] organizations.schouw_camera_standaard toegevoegd.")
...
app.include_router(car2023_router.router)
...
app.include_router(incidenten_router.router)
app.include_router(wpi_router.router)
app.include_router(lmra_router.router)
app.include_router(bouw_router.router)
app.include_router(schouw_router.router)
app.include_router(kwaliteit_router.router)
